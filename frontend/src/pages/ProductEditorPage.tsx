...
import { changeDraft, Drafts, draftValue, EDITOR_COLUMNS, EditorColumn, fieldPath, normalizeEditorValue, originalValue, parseEditorTsv } from './productEditorGrid';
...
export function ProductEditorPage() {
  ...
  const [visible, setVisible] = useState(() => new Set(EDITOR_COLUMNS.filter(column => !column.hidden).map(column => column.key)));
  const columns = EDITOR_COLUMNS.filter(column => visible.has(column.key) && (!column.modes || column.modes === (mode === 'common' ? 'common' : 'shop')));
  ...
  const selectedRows = data?.items.filter(row => selected.has(row.id)) || [];
  ...
  function move(id: number, field: string, dr: number, dc: number, skipLocked = false) {
    ...
    let r = data.items.findIndex(row => row.id === id), c = columns.findIndex(column => column.key === field);
    if (skipLocked) {
      for (let n = 0; n < data.items.length * columns.length; n++) {
        c += dc; if (c >= columns.length) { c = 0; r++; } if (c < 0) { c = columns.length - 1; r--; }
        if (r < 0 || r >= data.items.length) return;
        if (editable(columns[c])) { focusCell(data.items[r].id, columns[c].key); return; }
      }
    } else focusCell(data.items[Math.max(0, Math.min(data.items.length - 1, r + dr))].id, columns[Math.max(0, Math.min(columns.length - 1, c + dc))].key);
  }
  ...
  function paste(event: React.ClipboardEvent, row: ProductEditorRow, column: EditorColumn) {
    ...
    const startRow = data.items.findIndex(item => item.id === row.id), startColumn = columns.findIndex(item => item.key === column.key);
    if (!matrix || startRow + matrix.length > data.items.length || startColumn + matrix[0].length > columns.length) { setError('paste_bounds'); return; }
    ...
    for (let r = 0; r < matrix.length; r++) for (let c = 0; c < matrix[r].length; c++) {
      ...
      next = changeDraft(next, data.items[startRow + r], target.key, mode, value.value);
    }
  ...
  function display(row: ProductEditorRow, column: EditorColumn) {
    ...
    if (['on_hand', 'reserved', 'available', 'cost', 'sale_price_gross', 'shop_price', 'vat_rate'].includes(column.key)) return price(value);
  ...
  return <div className="product-editor">
    ...
    {!hubUnlocked() ? <form className="product-editor-unlock" onSubmit={event => { event.preventDefault(); if (token.trim()) unlockHub(token.trim()); }}><label>{t('productEditor.token')}<input data-testid="unlock-token" type="password" autoComplete="off" value={token} onChange={event => setToken(event.target.value)} /></label><p>{t('productEditor.tokenHelp')}</p><Button data-testid="unlock" type="submit" disabled={!token.trim()}>{t('productEditor.unlock')}</Button></form> : <>
      ...
      </div><span className="product-editor-price-basis">EUR · {t('productEditor.inclVat')}</span><Button data-testid="columns" variant="ghost" size="sm" icon={<Columns size={15} />} onClick={() => setShowColumns(value => !value)}>{t('productEditor.columns')}</Button></div>
      {showColumns && <div className="product-editor-columns">{EDITOR_COLUMNS.filter(column => !column.modes || column.modes === (mode === 'common' ? 'common' : 'shop')).map(column => <label key={column.key}><input data-testid={`column-${column.key}`} type="checkbox" checked={visible.has(column.key)} disabled={column.key === 'sku'} onChange={event => { finishEdit(); setActive(null); setVisible(previous => { const next = new Set(previous); event.target.checked ? next.add(column.key) : next.delete(column.key); return next; }); }} />{t(`productEditor.fields.${column.key}`)}</label>)}</div>}
      <form className="product-editor-filters" onSubmit={event => { event.preventDefault(); load(); }}>
        ...
        <Button data-testid="load" type="submit" variant="secondary" disabled={!!busy || uncertain}>{t(busy === 'load' ? 'productEditor.loading' : 'productEditor.load')}</Button>
      </form>
      <div className="product-editor-savebar"><div><strong>{dirtyCount ? t('productEditor.dirtyCount', { count: dirtyCount }) : t('productEditor.noChanges')}</strong><small>{t('productEditor.localOnly')}</small></div>
        <div><Button data-testid="discard" variant="ghost" disabled={!dirtyCount || locked} onClick={() => { editingRef.current = null; setEditing(null); replaceDrafts({}); setError(''); }}>{t('productEditor.discard')}</Button><Button data-testid="save" icon={<Save size={15} />} disabled={!dirtyCount || !!busy || uncertain} onClick={save}>{t('productEditor.save')}</Button></div></div>
      ...
      {selectedRows.length > 0 && <div className="product-editor-bulk"><strong>{t('productEditor.selectedPage', { count: selectedRows.length })}</strong><select data-testid="bulk-field" aria-label={t('productEditor.bulkField')} disabled={locked || !!busy} value={bulkField} onChange={event => { setBulkField(event.target.value); setBulkValue(''); }}>{columns.filter(editable).map(column => <option key={column.key} value={column.key}>{t(`productEditor.fields.${column.key}`)}</option>)}</select>
        ...
        <Button data-testid="bulk-apply" variant="secondary" size="sm" disabled={locked || !!busy} onClick={bulk}>{t('productEditor.bulkApply')}</Button><button className="product-editor-text-button" onClick={() => setSelected(new Set())}>{t('productEditor.clearSelection')}</button></div>}
      <div className={`product-editor-workspace ${detailId ? 'with-detail' : ''}`}><div className="product-editor-table-area">
        {!data ? <div className="product-editor-empty">{t('productEditor.initialHelp')}</div> : <>
          <div className="product-editor-grid-scroll" data-testid="grid-scroll"><table role="grid" aria-label={t('productEditor.title')} aria-rowcount={data.total + 1}>
            <thead><tr><th className="product-editor-select"><input data-testid="select-page" type="checkbox" aria-label={t('productEditor.selectPage')} checked={data.items.length > 0 && data.items.every(row => selected.has(row.id))} disabled={locked} onChange={event => setSelected(new Set(event.target.checked ? data.items.map(row => row.id) : []))} /></th>{columns.map(column => <th key={column.key} className={`product-editor-col-${column.key}`}>{t(`productEditor.fields.${column.key}`)}{editable(column) && <span className="product-editor-pencil" aria-hidden="true">✎</span>}</th>)}<th aria-label={t('productEditor.detail')} /></tr></thead>
            <tbody>{data.items.map((row, index) => <tr key={row.id} className={`${selected.has(row.id) ? 'is-selected' : ''} ${row.group && index > 0 && data.items[index - 1].group?.id !== row.group.id ? 'product-editor-group-start' : ''}`}>
              ...
              {columns.map(column => {
                ...
                return <td key={column.key} ref={element => { if (element) cells.current.set(`${row.id}:${column.key}`, element); else cells.current.delete(`${row.id}:${column.key}`); }} data-testid={`cell-${row.id}-${column.key}`} data-dirty={dirty || undefined} aria-readonly={!editable(column)} aria-invalid={!!code} tabIndex={active ? active.id === row.id && active.field === column.key ? 0 : -1 : index === 0 && column.key === 'sku' ? 0 : -1}
                  ...
                  {isEditing ? column.type === 'visibility' ? <select data-testid="cell-editor" ref={element => { editor.current = element; }} value={editing.value} onChange={event => { const next = { ...editing, value: event.target.value }; editingRef.current = next; setEditing(next); }} onBlur={() => finishEdit()} onKeyDown={event => { if (['Enter', 'Tab', 'Escape'].includes(event.key)) { event.preventDefault(); event.stopPropagation(); finishEdit(event.key === 'Tab' ? event.shiftKey ? -1 : 1 : 0, event.key === 'Escape'); } }}><option value="">{t('productEditor.inherit')}</option><option value="true">{t('productEditor.visible')}</option><option value="false">{t('productEditor.hidden')}</option></select> : <input data-testid="cell-editor" ref={element => { editor.current = element; }} value={editing.value} inputMode={column.type === 'money' || column.type === 'vat' ? 'decimal' : column.type === 'integer' ? 'numeric' : 'text'} onChange={event => { const next = { ...editing, value: event.target.value }; editingRef.current = next; setEditing(next); }} onBlur={() => finishEdit()} onKeyDown={event => { if (['Enter', 'Tab', 'Escape'].includes(event.key)) { event.preventDefault(); event.stopPropagation(); finishEdit(event.key === 'Tab' ? event.shiftKey ? -1 : 1 : 0, event.key === 'Escape'); } }} /> : <>
                    <span className="product-editor-cell-value">{display(row, column)}</span>{column.key === 'sku' && row.attributes.length > 0 && <small>{row.attributes.map(attribute => attribute.value).join(' / ')}</small>}{code && <small className="product-editor-cell-error">{message(code)}</small>}
                  </>}
                </td>;
