  ...
  return <div className="opening-stock stock-publication">
    ...
    {!hubUnlocked() ? <form className="opening-stock-panel opening-stock-unlock" onSubmit={event => { event.preventDefault(); if (token.trim()) unlockHub(token.trim()); }}>
      ...
      <section className="opening-stock-panel"><div className="opening-stock-actions"><label>{t('stockPublication.shop')}<select data-testid="shop" disabled={writing} value={shop} onChange={event => { reset(); setShop(event.target.value); }}><option value="biketrek">BIKETREK</option><option value="xtrek">xTrek</option></select></label>
        <span className="action-control"><Button data-testid="load-options" variant="secondary" disabled={!!busy} onClick={() => read('options', signal => getPublicationOptions(shop, signal), acceptOptions)}>{t('stockPublication.loadOptions')}</Button><ActionScope effects={['hub-read']} /></span></div>
      ...
      {options && <>
        <section className="opening-stock-panel"><h2>{t('stockPublication.policyTitle')}</h2><p>{t('stockPublication.warehouse')}: {options.warehouse.name} · {options.warehouse.code}</p>
          ...
          <span className="action-control"><Button data-testid="save-policy" disabled={!!busy || !!uncertain || !policyConfirm || dangerousBatch} onClick={configure}>{t('stockPublication.savePolicy')}</Button><ActionScope effects={['hub-write']} /></span>
        </section>
        <section className="opening-stock-panel"><h2>{t('stockPublication.holdTitle')}</h2><p>{t('stockPublication.holdHelp')}</p>
          {activeHold ? <>
            ...
            <span className="action-control"><Button data-testid="release-hold" variant="secondary" disabled={!!busy || !!uncertain || dangerousBatch || !maintenanceCompleted || activeHold.shop_code !== shop} onClick={releaseHold}>{t('stockPublication.releaseHold')}</Button><ActionScope effects={['hub-write']} /></span>
            ...
            <span className="action-control"><Button data-testid="create-hold" disabled={!!busy || !!uncertain || !writersPaused || !ordersReconciled} onClick={openHold}>{t('stockPublication.createHold')}</Button><ActionScope effects={['hub-write']} /></span>
          </>}
        </section>
        <section className="opening-stock-panel"><h2>{t('stockPublication.previewTitle')}</h2><p>{t('stockPublication.previewHelp', { limit: options.values.publication_batch_size, minutes: options.values.publication_preview_minutes })}</p>
          ...
          <span className="action-control"><Button data-testid="preview" disabled={!!busy || !!uncertain || !activeHold || activeHold.shop_code !== shop || !skuText.trim() || dangerousBatch || policyDirty} onClick={prepare}>{t('stockPublication.preview')}</Button><ActionScope effects={['upgates-read', 'hub-write']} shop={shop} calls={{ kind: 'variable' }} /></span>
      ...
      <section className="opening-stock-panel"><h2>{t('stockPublication.recoveryTitle')}</h2><p>{t('stockPublication.recoveryHelp')}</p>
        <div className="opening-stock-actions"><label className="opening-stock-grow">{t('stockPublication.batchId')}<input data-testid="batch-id" value={batchId} disabled={writing || uncertain === 'batch'} onChange={event => {
          ...
          <span className="action-control"><Button data-testid="recover-batch" variant="secondary" disabled={!!busy || !uuidPattern.test(batchId)} onClick={() => recover()}>{t('stockPublication.recover')}</Button><ActionScope effects={['hub-read']} /></span>
          <span className="action-control"><Button data-testid="load-batches" variant="secondary" disabled={!!busy} onClick={() => loadRecent()}>{t('stockPublication.loadRecent')}</Button><ActionScope effects={['hub-read']} /></span></div>
        {recent && <div className="opening-stock-recent">{!recent.batches.length ? <p>{t('stockPublication.noRecent')}</p> : recent.batches.map(item => <div key={item.id}><span><code>{item.id}</code><small>{date(item.created_at)} · {state(item.status)}</small></span><span className="action-control"><Button size="sm" variant="secondary" disabled={!!busy || (uncertain === 'batch' && item.id !== batchId)} onClick={() => recover(item.id)}>{t('stockPublication.openBatch')}</Button><ActionScope effects={['hub-read']} /></span></div>)}
          <div className="opening-stock-pagination"><span className="action-control"><Button variant="secondary" size="sm" disabled={!!busy || recent.offset === 0} onClick={() => loadRecent(Math.max(0, recent.offset - recent.limit))}>{t('stockPublication.previous')}</Button><ActionScope effects={['hub-read']} /></span><span>{t('stockPublication.range', { from: recent.total ? recent.offset + 1 : 0, to: recent.offset + recent.batches.length, total: recent.total })}</span><span className="action-control"><Button variant="secondary" size="sm" disabled={!!busy || recent.offset + recent.limit >= recent.total} onClick={() => loadRecent(recent.offset + recent.limit)}>{t('stockPublication.next')}</Button><ActionScope effects={['hub-read']} /></span></div>
        </div>}
      </section>
      {batch && <section className="opening-stock-panel" aria-live="polite"><h2>{t('stockPublication.batchTitle')}</h2><p><strong>{state(batch.status)}</strong> · <code>{batch.id}</code></p><p className="opening-stock-muted">{t('stockPublication.expiresAt')}: {date(batch.expires_at)}</p>
        ...
        {batch.status === 'prepared' && <>
          ...
          <span className="action-control"><Button data-testid="submit" disabled={!!busy || !!uncertain || expired || !sameHold || !options?.server_write_enabled || !options.external_write_enabled || !submitConfirm || policyDirty} onClick={submit}>{t('stockPublication.submit')}</Button><ActionScope effects={['hub-write', 'queued-upgates', 'upgates-read', 'upgates-write']} shop={shop} calls={{ kind: 'variable' }} /></span>
        </>}
        ...
        <div className="opening-stock-actions stock-publication-result-actions">
          <span className="action-control"><Button data-testid="verify" variant="secondary" disabled={!!busy || !!uncertain || !sameHold || ['queued', 'running'].includes(batch.status)} onClick={() => { if (sameHold) mutate('verify', () => verifyPublication(batch.id), acceptBatch, 'batch'); }}>{t('stockPublication.verify')}</Button><ActionScope effects={['upgates-read', 'hub-write']} shop={shop} calls={{ kind: 'variable' }} /></span>
          <span className="action-control"><Button variant="secondary" disabled={!!busy} onClick={() => recover(batch.id)}>{t('stockPublication.recover')}</Button><ActionScope effects={['hub-read']} /></span>
        </div>
        {batch.status === 'uncertain' && <><div className="opening-stock-confirmations"><label><input data-testid="requests-finished" type="checkbox" checked={requestsFinished} disabled={!!busy || !!uncertain || !sameHold} onChange={event => setRequestsFinished(event.target.checked)} />{t('stockPublication.requestsFinished')}</label></div><span className="action-control"><Button data-testid="resolve" disabled={!!busy || !!uncertain || !requestsFinished || !sameHold} onClick={() => { if (requestsFinished && sameHold) mutate('resolve', () => resolvePublication(batch.id), acceptBatch, 'batch'); }}>{t('stockPublication.resolve')}</Button><ActionScope effects={['upgates-read', 'hub-write']} shop={shop} calls={{ kind: 'variable' }} /></span></>}
        {['prepared', 'queued'].includes(batch.status) && <><div className="opening-stock-confirmations"><label><input data-testid="cancel-confirm" type="checkbox" disabled={!!busy || !!uncertain} checked={cancelConfirm} onChange={event => setCancelConfirm(event.target.checked)} />{t('stockPublication.cancelConfirm')}</label></div><span className="action-control"><Button data-testid="cancel" variant="secondary" disabled={!!busy || !!uncertain || !cancelConfirm} onClick={() => { if (cancelConfirm) mutate('cancel', () => cancelPublication(batch.id), acceptBatch, 'batch'); }}>{t('stockPublication.cancel')}</Button><ActionScope effects={['hub-write']} /></span></>}
      </section>}
