...
export interface AiRule { id: string; name: string; scope: AiScope; instructions: string; policy: AiPolicy; enabled: boolean; official_domains: string[] }
...
export interface AiJob { id: string; batch_id: string; kind: 'product' | 'rules'; status: string; revision: number; shop: string; supplier: string; code: string; name: string; image: string | null; product_ids: number[]; use_ai: boolean; rules_version: number; category_profile: string; policy: AiPolicy; origins: Record<string, string>; estimate_usd: string; actual_usd: string | null; reserved_usd: string; checks: { errors?: string[]; warnings?: string[] }; error: string | null; preview_id: string | null; created_at: string; updated_at: string; output?: AiContent & { instructions?: string; reason?: string; questions?: string[] }; facts?: { id: number; name: string; description: string; variant_attributes: { name: string; value: string }[] }[]; events?: { at: string; status: string; note: string }[]; preview?: ImportPreview; import_result?: ImportResult; options?: ImportOptions; research?: string }
