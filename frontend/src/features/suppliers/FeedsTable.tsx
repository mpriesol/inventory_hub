...
export function FeedsTable({ files, onTogglePreview, previews }: {
  files: ListedFile[];
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-sm text-white/80">Feeds / Converted</h3>
        <Pill>{files.length}</Pill>
      </div>
      {!files.length && <div className="text-sm text-white/60">Žiadne súbory.</div>}
      {!!files.length && (
        <NiceTable>
          {files.map((f) => {
