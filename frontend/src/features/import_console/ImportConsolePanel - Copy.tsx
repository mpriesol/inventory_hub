...
export default function ImportConsolePanel({ supplier, shop, defaultInvoiceId }: Props) {
  ...
  React.useEffect(() => {
    ...
    fetchCsvOutputs(supplier, invParam).then(async (d) => {
      ...
      setPage(0);
      // pri zmene tabu/faktúry resetni edit draft
      setEdits({});
  ...
  const getCellOriginal = (row: any[], colName: string): string => {
    const exactIdx = (preview?.columns || []).indexOf(colName);
    const idx = exactIdx >= 0 ? exactIdx : (preview?.columns || []).findIndex(h => norm(h) === norm(colName));
  ...
  const fmtSigned = (x: number): string => {
    ...
    // prípadne tu neskôr doplníme kurz/prefixy
  };
