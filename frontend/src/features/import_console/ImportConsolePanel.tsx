...
export default function ImportConsolePanel({ supplier, shop, defaultInvoiceId }: Props) {
  ...
  const [receivingOpen, setReceivingOpen] = React.useState(false);
  ...
  // Receiving → pre-naplní selection a draft edity (INVOICE_QTY) pre aktuálny tab
  const handleReceivingSelection = (payload: { selected_product_codes: string[], edits: Record<string, Record<string,string>> }) => {
    const codes = Array.from(new Set(payload?.selected_product_codes || []));
    // 1) merge selection to current tab
    setSelectedByTab(prev => {
      const copy = new Set(prev[tab]);
      codes.forEach(c => copy.add(c));
      return { ...prev, [tab]: copy };
    });
    // 2) merge edits (norm keys)
    setEdits(prev => {
      const next = { ...prev };
      for (const code of codes) {
        const src = payload.edits?.[code] || {};
        const normed: Record<string,string> = {};
        for (const k of Object.keys(src)) normed[norm(k)] = String(src[k] ?? "");
        next[code] = { ...(next[code] || {}), ...normed };
      }
      return next;
    });
    ...
    <Section title="Import Console">
      <div className="flex flex-wrap items-center gap-2 mb-3">
        ...
        <div className="ml-auto flex flex-wrap items-center gap-2">
          ...
          <OutlineButton onClick={()=>setReceivingOpen(true)}>Receiving</OutlineButton>
      ...
      {receivingOpen && (
        <div className="fixed inset-0 z-[210]">
          <div className="absolute inset-0 bg-black/70" onClick={() => setReceivingOpen(false)} />
          <div
            className="relative mx-auto mt-[5vh] w-[92vw] h-[88vh] rounded-2xl
                       bg-neutral-900 text-neutral-100 shadow-2xl border border-neutral-800
                       overflow-hidden"
          >
            <div className="flex items-center justify-between px-4 py-3 border-b border-neutral-800">
              <div className="font-semibold">Receiving — {invoice}</div>
              <button
            ...
            <div className="p-2 h-[calc(88vh-56px)] overflow-auto">
              <ReceivingPanel
                supplier={supplier}
                invoiceId={(invoice && invoice.includes(':')) ? (invoice as string) : `${supplier}:${invoice}`}
                onUseSelection={handleReceivingSelection}
              />
            </div>
          </div>
        </div>
