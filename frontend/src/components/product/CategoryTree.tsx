...
export function CategoryTree({ categories, value, language = 'sk', onChange }: {
  ...
  const matches = categories.filter(c => normalize(path(c.code) + c.code).includes(normalize(search)));
  const draw = (parent: string | null, seen = new Set<string>()): React.ReactNode => categories.filter(c => (c.parent_code && index.has(c.parent_code) ? c.parent_code : null) === parent).map(c => {
    ...
    const button = <button type="button" aria-pressed={value === c.code} className={value === c.code ? 'ai-primary' : ''} onClick={() => onChange(c.code)}>{name(c.code)} <small>{c.code}{c.active === false ? ` · ${t('ai.inactiveCategory')}` : ''}</small></button>;
