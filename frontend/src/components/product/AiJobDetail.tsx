...
export function AiJobDetail({ job, onChange }: { job: AiJob; onChange: (job: AiJob) => void }) {
  ...
  const canFork = job.kind === 'product' && !job.update_only && !unknownImport && !['queued','generating','preparing_import','import_queued','importing'].includes(job.status);
  ...
  return <section className="ai-card">
    <div className="ai-row"><h2 className="ai-grow">{job.name} · {job.shop}</h2><span className="ai-badge">{t(job.update_only && job.status === 'exists' ? job.update_preview?.state === 'completed' ? 'ai.updatedExisting' : 'ai.readyForUpdate' : `ai.states.${job.status}`, { defaultValue: job.status })}</span></div>
    <p>{job.code} · {t('ai.publishedVersion', { version: job.rules_version })} · {job.category_profile}</p>
    <div className="ai-next-step"><strong>{t('ai.nextStep')}</strong><p>{t(job.update_only ? `ai.updateNext.${job.update_preview?.state === 'completed' ? 'completed' : job.status}` : `ai.next.${job.status}`, { defaultValue: t(`ai.next.${job.status}`, {defaultValue:job.status}) })}</p></div>
    ...
    {['import_blocked','exists','completed','cancelled'].includes(job.status) && job.output && <button disabled={busy} onClick={() => action('reopen')}>{t('ai.reopenContent')}</button>}
    ...
    {job.kind === 'product' && content && ['review','blocked','ready','exists','completed','import_blocked'].includes(job.status) && <details open={job.update_only || undefined}><summary>{t('ai.updateExisting')}</summary><p>{t('ai.updateExistingHelp')}</p><div className="ai-toolbar">{['title','short_description','long_description','seo_title','seo_description','metas','parameters','categories','availability'].map(field => <label className="ai-check" key={field}><input type="checkbox" disabled={busy || (field === 'availability' && job.source_kind === 'shop')} checked={updateFields.includes(field)} onChange={e => setUpdateFields(old => e.target.checked ? [...old,field] : old.filter(f => f !== field))} />{t(`ai.updateFields.${field}`)}</label>)}</div>{job.source_kind === 'shop' && <p>{t('ai.shopAvailabilityHelp')}</p>}<button disabled={busy || dirty || !updateFields.length} onClick={() => perform(() => aiRequest(`/jobs/${job.id}/update-preview`, {expected_revision:job.revision,fields:updateFields}))}>{t('ai.prepareUpdate')}</button></details>}
    {job.update_preview && <div className="ai-card"><h3>{t('ai.updateComparison')}</h3><p>{t(`ai.updateStates.${job.update_preview.state}`)}</p>{job.update_preview.fields.map(field => <details key={field}><summary>{t(`ai.updateFields.${field}`)}</summary><div className="ai-grid"><div><h4>{t('ai.beforeUpdate')}</h4><UpdateValue data={job.update_preview!.before} field={field} language={job.options?.language || 'sk'} title={`${t('ai.beforeUpdate')} · ${t(`ai.updateFields.${field}`)}`} /></div><div><h4>{t('ai.afterUpdate')}</h4><UpdateValue data={job.update_preview!.after} field={field} language={job.options?.language || 'sk'} title={`${t('ai.afterUpdate')} · ${t(`ai.updateFields.${field}`)}`} /></div></div></details>)}{job.update_result?.error && <div className="ai-notice ai-error" role="alert">{t(`ai.errors.${job.update_result.error}`, {defaultValue:job.update_result.error})}</div>}{!['completed','rejected'].includes(job.update_preview.state) && <button disabled={busy || dirty} className="ai-primary" onClick={() => perform(() => aiRequest(`/jobs/${job.id}/update-confirm`, {expected_revision:job.revision,preview_id:job.update_preview!.id}))}>{t(job.update_preview.state === 'ready' ? 'ai.confirmUpdate' : 'ai.reconcileUpdate')}</button>}</div>}
