...
export function FifoPanel({ productId, warehouseCode, onChanged }: { productId: number; warehouseCode: string; onChanged?: () => void }) {
  ...
  return <section className="fifo-panel" data-testid="fifo-panel">
    <div className="fifo-toolbar"><h3>{c.title}</h3><label>{c.warehouse}<select data-testid="fifo-warehouse" value={warehouse} disabled={busy || !!pending} onChange={e => setWarehouse(e.target.value)}><option value="">{c.chooseWarehouse}</option>{warehouses.map(w => <option key={w.id} value={w.code}>{w.name}</option>)}</select></label><button type="button" disabled={busy || loading} onClick={() => setRefresh(v => v + 1)}>{c.refresh}</button></div>
    {error && <p role="alert">{error}</p>}{message && <p role="status">{message}</p>}
    {pending && <div role="alert"><p>{c.pending}</p><button type="button" data-testid="fifo-retry" disabled={busy} onClick={() => execute(pending)}>{c.retry}</button></div>}
    {loading && <p role="status">{c.loading}</p>}
    {stock && <>
      {stock.valuation.mode !== 'fifo' && <p className="fifo-notice">{stock.valuation.mode === 'legacy' ? c.legacy : c.missing}</p>}
      ...
      {operation && <div className="fifo-form" data-testid="fifo-form"><h4>{c[operation]}</h4><p>{c[`${operation}Help` as 'receiveHelp']}</p>
        ...
        <div className="fifo-toolbar"><button type="button" data-testid="fifo-submit" disabled={busy || !!pending || !confirmed || (operation === 'return' && !returnData) || (operation === 'revise' && !costData)} onClick={submit}>{!preview && ['receive', 'cutover'].includes(operation) ? c.preview : c.apply}</button><button type="button" disabled={busy || !!pending} onClick={reset}>{c.cancel}</button></div>
      </div>}
      <h4>{c.layers}</h4><div className="fifo-scroll"><table><thead><tr><th>{c.date}</th><th>{c.source}</th><th>{c.remaining}</th><th>{c.cost}</th><th>{c.state}</th><th /></tr></thead><tbody>{stock.layers.map(layer => <tr key={layer.id}><td>#{layer.id}<br />{new Date(layer.physical_received_at).toLocaleString()}</td><td>{String(layer.provenance.source_reference || layer.provenance.case_reference || '')}</td><td>{layer.quantity_remaining} / {layer.quantity_original}</td><td>{amount(layer.unit_cost)}</td><td>{status(layer.cost_status)} · {layer.stock_status === 'quarantine' ? c.quarantine : c.available}</td><td><button type="button" data-testid={`fifo-revise-${layer.id}`} disabled={busy || !!pending} onClick={() => inspectCost(layer)}>{c.revise}</button>{layer.stock_status === 'quarantine' && Number(layer.quantity_remaining) > 0 && <button type="button" data-testid={`fifo-release-${layer.id}`} disabled={busy || !!pending} onClick={() => open('release', layer)}>{c.release}</button>}</td></tr>)}</tbody></table></div>
      {stock.layers.length === 0 && <p>{c.noLayers}</p>}<div className="fifo-toolbar"><button type="button" disabled={loading || layerOffset === 0} onClick={() => setLayerOffset(v => Math.max(0, v - 50))}>{c.previous}</button><span>{Math.min(layerOffset + 50, stock.total)} / {stock.total}</span><button type="button" disabled={loading || layerOffset + 50 >= stock.total} onClick={() => setLayerOffset(v => v + 50)}>{c.following}</button></div>
      <h4>{c.history}</h4><div className="fifo-scroll"><table><thead><tr><th>{c.date}</th><th>{c.state}</th><th>{c.quantity}</th><th>{c.originalCost}</th><th /></tr></thead><tbody>{movements.map(row => <tr key={row.id}><td>{new Date(row.created_at).toLocaleString()}</td><td>{row.movement_type} #{row.id}<br />{row.reference_id}</td><td>{row.quantity}</td><td>{amount(row.total_cost)}</td><td>{row.movement_type.toUpperCase() === 'SALE_OUT' && <button type="button" data-testid={`fifo-return-${row.id}`} disabled={busy || !!pending} onClick={() => inspectIssue(row.id)}>{c.inspect}</button>}</td></tr>)}</tbody></table></div>
      {movements.length === 0 && <p>{c.noMovements}</p>}<div className="fifo-toolbar"><button type="button" disabled={loading || historyOffset === 0} onClick={() => setHistoryOffset(v => Math.max(0, v - 50))}>{c.previous}</button><span>{Math.min(historyOffset + 50, historyTotal)} / {historyTotal}</span><button type="button" disabled={loading || historyOffset + 50 >= historyTotal} onClick={() => setHistoryOffset(v => v + 50)}>{c.following}</button></div>
    </>}
  </section>;
}
