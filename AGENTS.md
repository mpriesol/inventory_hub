...
Scanner UUIDs identify individual operations, never an EAN or a time window. Ten fresh scans mean ten increments. Preserve successful UUID results atomically with scan events, including after reset/finalization; only replay the same payload. Migration013 widens compound invoice EAN without truncation and rebuilds only its derived fingerprint in one transaction; unknown dependencies must fail without CASCADE. Do not roll back to UUID-unaware scanners as a retry recovery method. Receiving/catalog identity uses the shared resolver and revalidates stored product IDs; supplier SKU aliases remain supplier-scoped.
