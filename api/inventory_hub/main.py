...
from .utils import (
    ...
    upgates_output_names,
    read_csv_smart, clean_upgates_headers_inplace,   # <--- PRIDAŤ
)
...
from inventory_hub.routers.receiving import router as receiving_router

# ---------------------------------------------------------
# FastAPI app + CORS + /data static (servuje inventory-data)
# ---------------------------------------------------------
from inventory_hub.logging_setup import setup_logging
setup_logging(settings)

app = FastAPI(title="Inventory Hub API", version="1.0.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "http://localhost:5173",
