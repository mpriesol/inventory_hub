...
async def status(db, shop_code):
    ...
    fields = ("enabled", "revision", "cursor_at", "last_reconciled_at", "next_poll_at", "last_started_at",
              "last_completed_at", "last_error", "entries_seen")
    return {"shop": {"code": shop.code, "name": shop.name},
            ...
            "collector": {key: getattr(row, key) for key in fields} if row else None,
...
async def refresh(db, payload):
    ...
    if row is None or not row.enabled:
        raise CollectionError("order_collection_disabled")
    if row.revision != payload.expected_revision:
        raise CollectionError("order_collection_settings_changed")
    if (row.retry_after_at and now() < row.retry_after_at) or (row.last_started_at and now() < row.last_started_at + timedelta(seconds=60)):
...
async def runs(db, shop_code):
    ...
    return {"runs": [{key: getattr(row, key) for key in fields} for row in rows]}
...
async def start_run(db, shop_id):
    ...
    if not row or not row.enabled or row.next_poll_at > now():
        return None
    policy = await db.get(OrderStockPolicy, shop_id)
    warehouse = await db.get(Warehouse, policy.warehouse_id) if policy else None
    error = None
    ...
    if error:
        ...
        return None
    at = now()
    reconcile_due = row.last_reconciled_at is None or at - row.last_reconciled_at >= RECONCILE_INTERVAL
    ...
    if mode == "reconcile":
        ...
        until = min(start + RECONCILE_WINDOW, row.reconcile_until_at)
        created_from, changed_from = start, policy.starts_at
    else:
        start, until = max(policy.starts_at, row.cursor_at - OVERLAP), at
    ...
    run = OrderCollectionRun(id=str(uuid4()), shop_id=shop_id, settings_revision=row.revision, mode=mode,
        status="running", started_at=at, from_at=start, until_at=until, pages=0, observed_count=0)
    db.add(run)
    row.last_started_at, row.last_mode = at, mode
    row.next_poll_at = at + timedelta(seconds=POLL_SECONDS)
    await db.flush()
    return {"id": run.id, "shop_id": shop_id, "shop_code": shop.code, "target_fingerprint": row.target_fingerprint, "created_from": created_from,
            "changed_from": changed_from, "created_to": until}
...
async def _running(db, run_id):
    ...
    if run.status != "running" or not row.enabled or row.revision != run.settings_revision:
...
async def fail_run(db, run_id, code, retry_after=None):
    ...
    row.last_error = code
    row.failure_count += 1
    delay = min(3600, POLL_SECONDS * 2 ** min(row.failure_count - 1, 4))
    if type(retry_after) is int and retry_after > 0:
        delay = max(delay, min(retry_after, 604800))
    row.retry_after_at = at + timedelta(seconds=delay)
