...
def refresh_feed(supplier_base: Path, feed_config: Dict[str, Any]):
    ...
    out_name = _derive_out_name_from_stem(stem)
    out_csv  = conv_dir / out_name
    ...
    with out_csv.open("w", encoding="utf-8-sig", newline="") as f:
        ...
        for item in root.xpath("//SHOPITEM"):
            ...
            row = {col: "" for col in UPGATES_HEADER}
            row.update({
    ...
    # Optional: also write stem.csv for compatibility
    stem_csv = conv_dir / f"{stem}.csv"
    if not stem_csv.exists():
        with stem_csv.open("w", encoding="utf-8-sig", newline="") as f:
            w = csv.DictWriter(f, fieldnames=UPGATES_HEADER, delimiter=";")
            w.writeheader()
            for row in csv.DictReader(open(out_csv, encoding="utf-8-sig"), delimiter=";"):
                w.writerow(row)
